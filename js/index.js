...
var gameObject = {
    ...
    formCheck: function(phraseToCheck){
        if(phraseToCheck.length <2 || phraseToCheck.length>3 || !(this.validator.test(phraseToCheck))  ){
              ...
              var log = document.getElementById('logScreen');
                ...
                log.innerHTML = "Invalid coordinates! <br><br>Please enter value in (A-J)(1-10) format.";
           return true;
        }
    ...
    shotBefore: function (check) {
        for (i = 0; i < this.shotsMade.length; i++) {
            if (check == this.shotsMade[i]) {
                ...
                var log = document.getElementById('logScreen');
                log.setAttribute('class', 'warning');
                log.innerHTML = "You've already triend this value. Please enter another one.";
    ...
    isGameOver: function(){
        if(this.sunk == warshipObject.noOfShips){
            var log = document.getElementById('logScreen');
            log.setAttribute('class', 'info');
            log.innerHTML = "GAME OVER <BR> Congratulations! <br><br> Number of shots made: " +this.shotsNumber + ".";
            return true;}
    }, // is GameOver end
    ...
    if(this.sunk == warshipObject.noOfShips){
            var log = document.getElementById('logScreen');
        log.setAttribute('class', 'info');
            log.innerHTML = "GAME OVER <BR><br>Try another time by refreshing the page.";
            return true;
    }
} // is GameOn end
